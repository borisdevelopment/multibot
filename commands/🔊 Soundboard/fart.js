...
module.exports = {
	...
	run: async (client, message, args, cmduser, text, prefix) => {
		...
		if (!channel) {return message.reply({embeds: [new MessageEmbed().setTitle('<:no:962755486938189844> You need to join a voice channel').setColor(es.wrongcolor).setFooter(client.getFooter(es))]});}
		...
		if (botchannel) {return message.reply({embeds: [new MessageEmbed().setTitle(`<:no:962755486938189844> I am already connected in: \`${botchannel.name}\``).setFooter(client.getFooter(es))]});}

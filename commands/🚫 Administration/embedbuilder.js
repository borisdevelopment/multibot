...
let buttonEvent = async (interaction) => {
  ...
  if (interaction?.message.id === msg.id) {
    if (interaction?.user.id === message.member.id) {
      ...
      await interaction?.reply({content : `<:no:962755486938189844> You are not allowed to do that! Only: <@${message.author.id}>`, ephemeral : true}); //ephemeral message
    }
  }
}
...
let embedEditing = async(button) => {
    ...
    if(id.startsWith(`builder`)) {
        ...
        button?.message.edit({content : `<a:loading:933964409661972491> **Please send me Your Input now!**`,

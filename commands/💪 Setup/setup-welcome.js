      ...
      async function handle_the_picks(optionhandletype, SetupNumber, menuoptiondata) {
        switch (optionhandletype) {          
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata){
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`:{
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<a:yes:962755522627514368> **Selected the \`${collected.first().customId}\` Color**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
                        });
                        //Menu Collections
                        collector.on('collect', async button => {
                          if (button?.user.id === cmduser.id) {
          ...
          case `Test Welcome`:{
            ...
            let invitemessage = `Invited by ${member.user.tag ? `**${member.user.tag}**` : `<@${member.user.id}>`}\n<:Like:934494916241948763> **X Invites**\n[<:joines:933723116822220821> X Joins | <:leave:933722870734024724> X Leaves | <:no:962755486938189844> X Fakes]`
            if(welcome){

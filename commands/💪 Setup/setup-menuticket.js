        ...
        switch (optionhandletype) {
            ...
            async function third_layer(SetupNumber) {
              ...
              collector.on('end', collected => {
                menumsg.edit({
                  embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                  components: [],
                  content: `${collected && collected.first() && collected.first().values ? `<a:yes:962755522627514368> **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`
                })
              });
            }
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:no:962755486938189844> **You need to add at least 1 Open-Ticket-Option**")
            }
          ...
          case "Edit Ticket Option": {
            let data = theDB.get(message.guild.id, "data");
            if (!data || data.length < 1) {
              return message.reply("<:no:962755486938189844> **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<a:yes:962755522627514368> **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
          break;

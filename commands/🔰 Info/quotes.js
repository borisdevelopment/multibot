    ...
    try {
      ...
      if(args[0] && !isNaN(args[0])){
        if(Number(args[0]) < 0 || Number(args[0]) > data.length - 1 || !data[Number(args[0])] || !data[Number(args[0])].text){
          return message.reply(`<:no:962755486938189844> **Invalid Quote ID!**\n> Use one between \`0\` and \`${data.length - 1}\``)
        }
      ...
      if(!data || data.length == 0) return message.reply({content: "<:no:962755486938189844> **This User has no Quotes in this Server yet!**"})

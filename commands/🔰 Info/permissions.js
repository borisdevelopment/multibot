  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
      ...
      try{
        ...
        embeduserinfo.setAuthor(handlemsg(client.la[ls].cmds.info.permissions.from, {usertag: member.user.tag}), member.user.displayAvatarURL({ dynamic: true }), "https://discord.com/api/oauth2/authorize?client_id=960185136752766997&permissions=8&scope=bot%20applications.commands")

...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      var button_disable = new MessageButton().setStyle('SECONDARY').setCustomId('notes_disable').setLabel('Stop the Buttons').setEmoji("962755486938189844");
      ...
      if(!notes || notes.length == 0){
          ...
          .setTitle(`<:no:962755486938189844> No Notes created yet`)
      ...
      try{
        if(b?.user.id !== message.author.id)
          ...
          if(b?.customId == "notes_disable") {
            ...
            var buttonRow2 = new MessageActionRow().addComponents([button_create, button_edit, button_Delete, button_disable])
            allbuttons = [buttonRow1, buttonRow2]
            await b?.reply("<:no:962755486938189844> **Disabled the Buttons**", true);
          ...
          if(b?.customId == "notes_jump") {
            ...
            await mmmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
              ...
              if(Page < 0 || Page > embeds.length - 1){
                return message.reply("<:no:962755486938189844> **Value out of Range!**").then(msg=>{
                  setTimeout(()=>{
                    try{msg.delete()}catch(e){console.log(String(e).grey)}
                  }, 2500)
                }).catch(e=>{
    ...
    collector.on('end', async collected => {
      if(!edited){
        ...
        var buttonRow2 = new MessageActionRow().addComponents([button_create.setDisabled(false), button_edit.setDisabled(false), button_Delete.setDisabled(false), button_disable.setDisabled(false)])
        allbuttons = [buttonRow1, buttonRow2]
        await notemsg.edit({content: `***<:no:962755486938189844> NOTE BUTTONS DISABLED***`,embeds: [embeds[currentPage]], components: allbuttons}).catch(e=>{console.log(String(e).grey)});
      }
    });
    } catch (e) {

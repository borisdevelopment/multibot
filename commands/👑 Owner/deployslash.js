...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (message.author.id != "921430546813419550")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setDescription(`Go to the [Discord-Server](https://discord.gg/zqjcg2vYRX), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:loading:933964409661972491> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:loading:933964409661972491> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)

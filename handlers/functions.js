...
function GetUser(message, arg){
  var errormessage = "<:no:962755486938189844> I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = "<:no:962755486938189844> I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
 ...
 */
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `<:no:962755486938189844> **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {

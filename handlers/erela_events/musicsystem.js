...
module.exports = client => {
    client.on("interactionCreate", async (interaction) => {
        ...
        if (!member.voice.channel)
            return interaction?.reply({
                content: `<:no:962755486938189844> **Please join a Voice Channel first!**`,
                ephemeral: true
            })      
            
        var player = client.manager.players.get(interaction?.guild.id);
        if (interaction?.customId != "Join" && interaction?.customId != "Leave" && (!player || !player.queue || !player.queue.current))
            return interaction?.reply({content: "<:no:962755486938189844> Nothing Playing yet", ephemeral: true})
        ...
        if (player && member.voice.channel.id !== player.voiceChannel)
            return interaction?.reply({
                content: `<:no:962755486938189844> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
                ephemeral: true
            })
        ...
        if(player && interaction?.customId != `Join` && interaction?.customId != `Lyrics` && check_if_dj(client, member, player.queue.current)) {
                return interaction?.reply({embeds: [new MessageEmbed()
                  ...
                  .setTitle(`<:no:962755486938189844> **You are not a DJ and not the Song Requester!**`)
        ...
        switch(interaction?.customId){
            ...
            case "Autoplay": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `<a:yes:962755522627514368> **Enabled Autoplay**`: `<:no:962755486938189844> **Disabled Autoplay**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Song": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `<a:yes:962755522627514368> **Enabled Song Loop**`: `<:no:962755486938189844> **Disabled Song Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Queue": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.queueRepeat ? `<a:yes:962755522627514368> **Enabled Queue Loop**`: `<:no:962755486938189844> **Disabled Queue Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
    ...
    client.on("messageCreate", async message => {
        ...
        if (!channel) return message.reply("<:no:962755486938189844> **Please join a Voice Channel first!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 5000)})
        ...
        if (player && channel.id !== player.voiceChannel) return message.reply(`<:no:962755486938189844> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
...
function generateQueueEmbed(client, guildId, leave){
    ...
    var embeds = [
      ...
      new MessageEmbed()
        ...
        .setImage(guild.banner ? guild.bannerURL({size: 4096}) : "https://i.ibb.co/KWZ9Hhf/NIYfa-Mb-Imgur.gif")
        .setTitle(`Start Listening to Music, by connecting to a Voice Channel and sending either the **SONG LINK** or **SONG NAME** in this Channel!`)
        .setDescription(`> *I support <:youtube:962755852975087686> Youtube, <:Spotify:962767868020293793> Spotify, <:soundcloud:934490678359121920> Soundcloud and direct MP3 Links!*`)
    ]

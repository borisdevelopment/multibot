...
for(let i = 1; i<=25; i++) {
  ...
  async function ApplySystem({ guild, channel, user, message, interaction, es, ls }) {
    ...
    try {
      ...
      interaction?.reply({content: `<a:yes:962755522627514368> Starting The Application In Your **Direct Messages!**`, ephemeral: true})
      ...
      function ask_question(qu) {
        ...
        user.send({embeds: [new Discord.MessageEmbed()
            ...
            .setDescription(qu)
            .setAuthor(client.getAuthor(`Question ${counter + 1} / ${Questions.length}`, client.user.displayAvatarURL(), "https://discord.gg/zqjcg2vyrx"))
            .setFooter(client.getFooter(es))
        ]}).then(msg => {
          ...
          if (interaction?.customId === "Apply_1") {
            ...
            try {
              ...
              if (roleid) {
                ...
                //try to add the role
                var member = message.guild.members.cache.get(usert.id)
                member.roles.add(roleid).catch((e)=>{channel_tosend.send("<:no:962755486938189844> I am Missing Permissions to grant the Role\n" + e.message)});
              }
            } catch (e) {
              //if an error happens, show it
